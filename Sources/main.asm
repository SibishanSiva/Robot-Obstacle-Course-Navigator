...
SENSOR_STBD     FCB $89
SENSOR_NUM      RMB 1
S_VAR           FCB $20 ; variance amount for sensors
...
CLEAR_LINE      FCC '                 '; MAYBE SHOULD BE 20??
               FCB NULL ; terminated by null
...
T_ADJ           ds.b 1 ; Time for adjustment turn
T_R_TRN         ds.b 1 ; Time to make a right turn at junction
TEN_THOUS       ds.b 1 ; 10,000 digit
THOUSANDS       ds.b 1 ; 1,000 digit
HUNDREDS        ds.b 1 ; 100 digit
TENS            ds.b 1 ; 10 digit
UNITS           ds.b 1 ; 1 digit
BCD_SPARE       ds.b 2 ;Extra space for decimal point and string terminator
...
NOT_REV_TRN       CMPA #R_ADJ
                 BNE NOT_R_ADJ
                 JSR ADJ_ST
                 BRA DISP_EXIT
                
NOT_R_ADJ         CMPA #L_ADJ
                 BNE NOT_L_ADJ
                 JSR ADJ_ST
                 BRA DISP_EXIT                 
   
NOT_L_ADJ         CMPA #R_TRN
                 BNE NOT_R_TRN
                 JSR R_TRN_ST
                 BRA DISP_EXIT
                                  
                
NOT_R_TRN         CMPA #L_TRN
                 BNE NOT_L_TRN
                 JSR L_TRN_ST
                 BRA DISP_EXIT
                         
NOT_L_TRN         CMPA #ALL_STP
                 BNE NOT_ALL_STP
                 JSR ALL_STP_ST
                 BRA DISP_EXIT
...
;---------------------------------------------------------------------------                 
;Forward state
FWD_ST            BRSET PORTAD0,$04,NO_FWD_BMP
                 JSR INIT_REV
                 LDY #6000
                 JSR del_50us
                 JSR INIT_R_TRN
...
NO_FWD_BMP        BRSET PORTAD0,$08,NO_REAR_BMP
                 JSR INIT_ALL_STP
                 MOVB #ALL_STP,CRNT_STATE
                JMP FWD_EXIT
                 
NO_REAR_BMP       LDAA SENSOR_BOW
                 ADDA VAR_BOW
                 CMPA BASE_BOW
                 BPL NO_ADJ
...
;------------------------------------------------
;Defualt: replace above NO_ADJ


;NO_ADJ            LDAA SENSOR_PORT
;                  ADDA VAR_PORT
;                  CMPA BASE_PORT
;                  BPL DO_L_TRN
;                  BMI NO_PORT 
...
DO_R_ADJ          JSR INIT_R_TRN
                 MOVB #R_ADJ,CRNT_STATE
                 BRA FWD_EXIT
                
DO_L_ADJ          JSR INIT_L_TRN
                 MOVB #L_ADJ,CRNT_STATE
                 BRA FWD_EXIT                     
...
INIT_R_TRN        BSET PORTA,%00000010
                 BCLR PORTA,%00000001
                 RTS
                
INIT_L_TRN        BSET PORTA,%00000001                 
                 BCLR PORTA,%00000010
                 RTS            
...
;---------------------------------------------------------------------------
; Initialize the ADC
openADC           MOVB #$C0,ATDCTL2 ; was 80 Turn on ADC (ATDCTL2 @ $0082)
                 LDY #1 ; Wait for 50 us for ADC to be ready
                 JSR del_50us ; - " -
                 MOVB #$00,ATDCTL3 ; was $20 4 conversions on channel AN1 (ATDCTL3 @ $0083)
                 MOVB #$85,ATDCTL4 ; was $97 8-bit resolution, prescaler=48 (ATDCTL4 @ $0084)
...
CON_EXIT      RTS ;We\92re done the conversion
...
C_TTHOU       LDAA TEN_THOUS ;Check the \92ten_thousands\92 digit
             ORAA NO_BLANK
             BNE NOT_BLANK1


ISBLANK1      LDAA #' ' ;It\92s blank
             STAA TEN_THOUS ;so store a space
             BRA C_THOU ;and check the \92thousands\92 digit


NOT_BLANK1    LDAA TEN_THOUS ;Get the \92ten_thousands\92 digit
             ORAA #$30 ;Convert to ascii
             STAA TEN_THOUS
             LDAA #$1 ;Signal that we have seen a \92non-blank\92 digit
             STAA NO_BLANK


C_THOU        LDAA THOUSANDS ;Check the thousands digit for blankness
             ORAA NO_BLANK ;If it\92s blank and \92no-blank\92 is still zero
             BNE NOT_BLANK2
...
NOT_BLANK2    LDAA THOUSANDS ;(similar to \92ten_thousands\92 case)
             ORAA #$30
             STAA THOUSANDS
             LDAA #$1
             STAA NO_BLANK
...
NOT_BLANK3    LDAA HUNDREDS ;(similar to \92ten_thousands\92 case)
             ORAA #$30
             STAA HUNDREDS
             LDAA #$1
             STAA NO_BLANK
...
NOT_BLANK4    LDAA TENS ;(similar to \92ten_thousands\92 case)
             ORAA #$30
             STAA TENS
